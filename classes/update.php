...
date_default_timezone_set("Africa/Lagos");
    class Update_table extends Dbh{
       ...
       public function updateAny($table, $data, $con, $val){
            $update = $this->connectdb()->prepare("UPDATE $table, $data WHERE $con = :$con");
            foreach($data as $column => $value){
                $update->bindValue($column, $value);
            }
            $update->bindValue("$con", $val);
            $update->execute();
       }
        ...
        public function update_double2cond($table, $column,$column2,  $condition, $condition2, $value, $value2, $condition_value, $condition_value2){
            $update = $this->connectdb()->prepare("UPDATE $table SET $column = :$column, $column2 = :$column2 WHERE $condition = :$condition AND $condition2 = :$condition2");
            $update->bindValue("$column", $value);
        ...
        //update sixteen
        public function update_sixteen($table, $column1, $value1, $column2, $value2, $column3, $value3, $column4, $value4, $column5, $value5, $column6, $value6, $column7, $value7, $column8, $value8, $column9, $value9, $column10, $value10, $column11, $value11, $column12, $value12,$column13, $value13, $column14, $value14, $column15, $value15, $column16, $value16,$condition, $condition_value){
            $update = $this->connectdb()->prepare("UPDATE $table SET $column1 = :$column1, $column2 = :$column2, $column3 = :$column3, $column4 = :$column4, $column5 = :$column5, $column6 = :$column6, $column7 = :$column7,  $column8 = :$column8, $column9 = :$column9, $column10 = :$column10, $column11 = :$column11, $column12 = :$column12, $column13 = :$column13, $column14 = :$column14, $column15 = :$column15, $column16 = :$column16 WHERE $condition = :$condition");
            $update->bindValue("$column1", $value1);
            $update->bindValue("$column2", $value2);
            $update->bindValue("$column3", $value3);
            $update->bindValue("$column4", $value4);
            ...
            $update->bindValue("$column7", $value7);
            $update->bindValue("$column8", $value8);
            $update->bindValue("$column9", $value9);
            $update->bindValue("$column10", $value10);
            $update->bindValue("$column11", $value11);
            $update->bindValue("$column12", $value12);
            $update->bindValue("$column13", $value13);
            $update->bindValue("$column14", $value14);
            $update->bindValue("$column15", $value15);
            $update->bindValue("$column16", $value16);
            $update->bindValue("$condition", $condition_value);
            $update->execute();
        }
        ...
        //update password
        public function updateUsername($username, $current_password, $user_id){
            $get_pwd = $this->connectdb()->prepare("SELECT user_password FROM users WHERE user_id = :user_id");
            $get_pwd->bindValue("user_id",$user_id);
            $get_pwd->execute();
            if($get_pwd->rowCount() > 0){
                ...
                if($confirm_password == false){
                    ...
                    $update_password = $this->connectdb()->prepare("UPDATE users SET username = :username WHERE user_id = :user_id");
                    $update_password->bindValue("user_id", $user_id);
                    $update_password->bindValue("username", $username);
                    $update_password->execute();

                    if($update_password){
                        echo "<p>Username updated Successfuly</p>";
                        session_start();
                        session_unset();
                        session_destroy();
                        
                    }else{
                        echo "<p class'exist'>Failed to update username</p>";
                    }
                }
        ...
        //merge customer
        public function mergeCustomer($table, $column, $correct, $wrong){
            $merge = $this->connectdb()->prepare("UPDATE $table SET $column = $correct WHERE $column = $wrong");
        ...
        public function update_data($table, $data, $where){
            // Build the SQL SET part dynamically
            $set_part = implode(', ', array_map(function($key) {
                return "$key = :$key";
            }, array_keys($data)));
    
            // Prepare SQL query for updating the data
            $sql = "UPDATE $table SET $set_part WHERE $where";
            $update_data = $this->connectdb()->prepare($sql);
