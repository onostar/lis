<?php
    ...
    class selects extends Dbh{
        //fetch dynamicall from any table with any condition
        public function select_data($table, $columns = "*", $conditions = []){
            // If specific columns are provided, implode them into a string; otherwise, select all (*)
            $columns_str = is_array($columns) ? implode(', ', $columns) : $columns;
    
            // Build the WHERE clause if conditions are provided
            $where_clause = "";
            if (!empty($conditions)) {
                $where_clause = " WHERE " . implode(' AND ', array_map(function($key) {
                    return "$key = :$key";
                }, array_keys($conditions)));
            }
    
            // Prepare the SQL query
            $sql = "SELECT $columns_str FROM $table $where_clause";
            $select_data = $this->connectdb()->prepare($sql);
    
            // Bind the condition values
            foreach($conditions as $column => $value){
                $select_data->bindValue(":$column", $value);
            }
            ...
            // Fetch the results
            return $select_data->fetchAll(PDO::FETCH_ASSOC);
        }
        //fetch the details with a condition and limit from a table desc
        public function fetch_limit_detailsDesc($table, $column, $value, $order, $limit){
        ...
        //fetch general parameter
        public function fetch_lab_template1($investigation, $gender, $age){
            $get_user = $this->connectdb()->prepare("SELECT * FROM lab_templates WHERE investigation = :investigation AND gender = :gender AND $age BETWEEN 'age_from' AND 'age_to'");
            $get_user->bindValue("investigation", $investigation);
            $get_user->bindValue("gender", $gender);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        //fetch age parameter
        public function fetch_lab_age_template($investigation, $age){
            $get_user = $this->connectdb()->prepare("SELECT * FROM lab_templates WHERE investigation = :investigation AND $age BETWEEN 'age_from' AND 'age_to'");
            $get_user->bindValue("investigation", $investigation);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        //fetch no parameter
        public function fetch_general_template($investigation){
            $get_user = $this->connectdb()->prepare("SELECT * FROM lab_templates WHERE investigation = :investigation AND gender = '' AND age_from = 0 AND age_to = 0");
            $get_user->bindValue("investigation", $investigation);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        //fetch gender parameter lab template
        public function fetch_lab_gender_template($investigation, $gender){
            $get_user = $this->connectdb()->prepare("SELECT * FROM lab_templates WHERE investigation = :investigation AND gender = :gender");
            $get_user->bindValue("investigation", $investigation);
            $get_user->bindValue("gender", $gender);
            $get_user->execute();
            if($get_user->rowCount() > 0){
        ...
        //1 condition is negative
        public function fetch_details_like1Condneg($table, $column, $value, $con, $conValue){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $con != :$con AND $column LIKE '%$value%'");
            $get_user->bindValue("$con", $conValue);
            $get_user->execute();
            if($get_user->rowCount() > 0){
        ...
        //fetch details count with 4 condition
        public function fetch_count_4cond($table, $column1, $condition1, $column2, $condition2, $col3, $val3, $col4, $val4){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $column1 = :$column1 AND $column2 = :$column2 AND $col3 = :$col3 AND $col4 = :$col4");
            $get_user->bindValue("$column1", $condition1);
            $get_user->bindValue("$column2", $condition2);
            $get_user->bindValue("$col3", $val3);
            $get_user->bindValue("$col4", $val4);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                return $get_user->rowCount();
            }else{
        ...
        //fetch details count with 4 condition and 1 negative
        public function fetch_count_4cond1neg($table, $column1, $condition1, $column2, $condition2, $col3, $val3, $col4, $val4, $neg, $negVal){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE $column1 = :$column1 AND $column2 = :$column2 AND $col3 = :$col3 AND $col4 = :$col4 AND $neg != :$neg");
            $get_user->bindValue("$column1", $condition1);
            $get_user->bindValue("$column2", $condition2);
            $get_user->bindValue("$col3", $val3);
            $get_user->bindValue("$col4", $val4);
            $get_user->bindValue("$neg", $negVal);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                return $get_user->rowCount();
            }else{
        ...
        //fetch details count with 1 condition  and 1 negative
        ...
        public function fetch_out_of_stock($store){
            $get_user = $this->connectdb()->prepare("SELECT * FROM inventory WHERE store = :store GROUP BY item HAVING SUM(quantity) = 0 ");
        ...
        public function fetch_out_of_stock_det($store){
            $get_user = $this->connectdb()->prepare("SELECT SUM(quantity) as total, item, cost_price FROM inventory WHERE store = :store GROUP BY item HAVING SUM(quantity) = 0");
            $get_user->bindValue("store", $store);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        public function fetch_count_curDate1Con($table, $column, $condition, $value){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE date($column) = CURDATE() AND $condition = :$condition");
            $get_user->bindValue("$condition", $value);
            $get_user->execute();
            if($get_user->rowCount() > 0){
        ...
        // select count with date and 2 positive condition
        public function fetch_count_curDate2Con($table, $column, $condition, $value, $con2, $val2){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE date($column) = CURDATE() AND $condition = :$condition AND $con2 = :$con2");
            $get_user->bindValue("$condition", $value);
        ...
        // select count with date and positive condition group
        public function fetch_count_curDate1ConGr($table, $column, $condition, $value, $group){
        ...
        public function fetch_count_curMonConGr($table, $column, $condition, $value, $group){
            $get_user = $this->connectdb()->prepare("SELECT * FROM $table WHERE MONTH($column) = MONTH(CURDATE()) AND $condition = :$condition GROUP BY $group");
            $get_user->bindValue("$condition", $value);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                return $get_user->rowCount();
        ...
        //fetch with current date and 2 condition
        public function fetch_details_curdate2Con($table, $column, $condition, $value, $con2, $val2){
        ...
        //fetch with two condition (one is negative)
        public function fetch_details_3cond1neg($table, $condition1, $condition2, $con3, $value1, $value2, $val3){
        ...
        public function fetch_details_curMonCon($table, $column, $condition, $value){
         ...
         //fetch all item grouped by a column
         public function fetch_single_groupedOrder($table, $group, $order){
            $get_details = $this->connectdb()->prepare("SELECT * FROM $table GROUP BY $group ORDER BY $order");
            $get_details->execute();
            if($get_details->rowCount() > 0){
                $row = $get_details->fetchAll();
                return $row;
        ...
        //fetch revenue with date
        public function fetch_revenue_new($store){
            $get_user = $this->connectdb()->prepare("SELECT SUM(amount_paid) AS total FROM payments WHERE store = :store AND date(post_date) = CURDATE()");
            $get_user->bindValue('store', $store);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        //fetch revenue with 2 dates
        public function fetch_revenueDatenew($from, $to, $store){
            $get_user = $this->connectdb()->prepare("SELECT SUM(amount_paid) AS total FROM payments WHERE store = :store AND date(post_date) BETWEEN '$from' AND '$to'");
            $get_user->bindValue('store', $store);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
        ...
        //fetch sum of column multiplied and current date with condition
        public function fetch_sum_2colCurDate2Con($table, $column1, $column2, $date, $condition, $value, $con2, $val2){
        ...
        public function fetch_count_reorderlevel($store){
            $get_user = $this->connectdb()->prepare("SELECT * FROM inventory WHERE store = :store GROUP BY item, reorder_level HAVING SUM(quantity) <= reorder_level");
            $get_user->bindValue("store", $store);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                return $get_user->rowCount();
        ...
        //fetch details with 2 negative condition
        public function fetch_private_service_tariff(){
            $get_user = $this->connectdb()->prepare("SELECT * FROM tariff WHERE category = 'Private' AND (item_group != 'Investigation' AND item_group != 'Pharmacy' AND item_group != 'Clinical Services')");
        ...
        //fetch hmo service tariffn
        public function fetch_hmo_service_tariff(){
            $get_user = $this->connectdb()->prepare("SELECT * FROM tariff WHERE category != 'Private' AND (item_group != 'Investigation' AND item_group != 'Pharmacy' AND item_group != 'Clinical Services')");
        ...
        //fetch hmo service tariffn
        public function fetch_sponsor_service_tariff($sponsor){
            $get_user = $this->connectdb()->prepare("SELECT * FROM tariff WHERE sponsor =:sponsor AND (item_group != 'Investigation' AND item_group != 'Pharmacy' AND item_group != 'Clinical Services')");
            $get_user->bindValue("sponsor", $sponsor);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $rows = $get_user->fetchAll();
                return $rows;
        ...
        //fetch single column details with single condition grouped
        /* public function fetch_details_groupOrder($table, $column, $condition, $value){
            $get_user = $this->connectdb()->prepare("SELECT $column FROM $table WHERE $condition = :$condition ORDER BY $column");
            $get_user->bindValue("$condition", $value);
            $get_user->execute();
            if($get_user->rowCount() > 0){
                $row = $get_user->fetch();
        ...
        public function fetch_monthly_sales($store){
            $get_monthly = $this->connectdb()->prepare("SELECT COUNT(distinct invoice) AS customers, SUM(amount_paid) AS revenue, post_date, COUNT(post_date) AS arrivals, COUNT(DISTINCT post_date) AS daily_average FROM payments WHERE store = :store GROUP BY MONTH(post_date) ORDER BY MONTH(post_date)");
