        ...
        <tr>
            ...
            <td><?php echo date("H:i:sa", strtotime($detail->post_date));?></td>
                ...
                <td style="color:var(--otherColor); text-align:center">
                    <?php 
                        //get transaction type;
                        if($detail->transaction == "purchase delete"){
                            echo $detail->quantity;
                        }else{
                            echo "0";
                        }
                ...
                <td style="color:var(--otherColor); text-align:center">
                    <?php 
                        //get transaction type;
                        if($detail->transaction == "dispense"){
                            echo $detail->quantity;
                        }else{
                            echo "0";
                        }
...
<?php
    ...
    if(gettype($details) == "array"){
    $get_qty = new selects();
    $totals = $get_qty->fetch_sum_double('inventory', 'quantity', 'item', $item, 'store', $store);
    if(gettype($totals) == "array"){
            foreach($totals as $total){
                echo "<p class='total_amount' style='color:green; text-align:center; text-decoration:underline'>Current quantity = ".$total->total."</p>";
