<?php
    ...
    $details = $get_visit->fetch_details_date2Con('visits', 'date(visit_date)', $from, $to, 'store', $store);
    $n = 1;  
?>
<h2>Patient Visits between '<?php echo date("jS M, Y", strtotime($from)) . "' and '" . date("jS M, Y", strtotime($to))?>'</h2>
    <hr>
    <div class="search">
        <input type="search" id="searchPurchase" placeholder="Enter keyword" onkeyup="searchData(this.value)">
        <a class="download_excel" href="javascript:void(0)" onclick="convertToExcel('data_table', 'Patient Visit Report')"title="Download to excel"><i class="fas fa-file-excel"></i></a>
    </div>
...
?>
            ...
            <td style="text-align:center; color:red;"><?php echo $n?></td>
                ...
                <td style="color:var(--moreColor)"><?php echo date("h:ia", strtotime($detail->visit_date))?></td>
                <td>
                    <?php
