<?php
    ...
    $details = $get_visit->fetch_details_date2Con('lab_results', 'date(post_date)', $from, $to, 'store', $store);
    $n = 1;  
?>
<h2>Investigations Done between '<?php echo date("jS M, Y", strtotime($from)) . "' and '" . date("jS M, Y", strtotime($to))?>'</h2>
    <hr>
    <div class="search">
        <input type="search" id="searchPurchase" placeholder="Enter keyword" onkeyup="searchData(this.value)">
        <a class="download_excel" href="javascript:void(0)" onclick="convertToExcel('data_table', 'Investigations Done')"title="Download to excel"><i class="fas fa-file-excel"></i></a>
    </div>
...
?>
            ...
            <td>
                ...
                <td>
                    <?php
                        //get investigation
                        $rows = $get_name->fetch_details_cond('items', 'item_id', $detail->investigation);
                ...
                <td>
                    <?php
                        //get sample
                        $samps = $get_name->fetch_details_2cond('sample_collection', 'visit_no', 'investigation', $detail->visit_number, $detail->investigation);
                        foreach($samps as $samp){
                        //get sample name
                            $rows = $get_name->fetch_details_cond('samples', 'sample_id', $samp->sample);
                            if(gettype($rows) == 'array'){
                                foreach($rows as $row){
                                    echo $row->sample;
                                }
