<?php
    ...
    if(isset($_GET['deposit_id'])){
        ...
        $bals = $get_balance->fetch_details_group('patients', 'wallet_balance', 'patient_id', $customer);
        $balance = $bals->wallet_balance;
        $new_balance = $balance - $amount;

        $update_wallet = new Update_table();
        $update_wallet->update('patients', 'wallet_balance', 'patient_id', $new_balance, $customer);
        if($update_wallet){
            //delete deposit
